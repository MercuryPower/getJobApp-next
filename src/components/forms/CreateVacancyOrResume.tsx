...
interface CreateVacancyOrResumeProps {
    onConfirm: (values: z.infer<typeof VacancyCreateSchema>) => void;
}
const CreateVacancyOrResume = ({onConfirm} : CreateVacancyOrResumeProps) => {
    const {user} = useAuth();
    ...
    const [cities, setCities] = useState<Option[]>(citiesForChoice)
    const [typesOfEmploy, setTypesOfEmploy] = useState<Option[]>(workTypes)
                ...
                <Dialog>
                    ...
                    <DialogContent className={'flex justify-center flex-col'}>
                        <DialogHeader>
                            ...
                            <Button className={'flex self-center bg-green-600 font-bo'} type={"submit"} onClick={form.handleSubmit(onConfirm)}>Подтвердить</Button>
                    </DialogContent>
                </Dialog>
...
export default CreateVacancyOrResume;