...
import ThemeSwitch from "@/entities/theme/ThemeSwitch";
...
import {BadgeAlert, BadgeCheck, CircleSlash2, CreditCard, FileText, Menu, ShieldAlert, User} from "lucide-react";
...
const Navbar = () => {
    ...
    return (
        <nav className={'flex justify-around  drop-shadow border-b'}>
            ...
            <Menu className={'h-10 w-10 cursor-pointer hover:bg-opacity-75 flex self-center md:hidden'} />
            <ul className={'hidden md:flex self-center justify-center '}>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button
                                ...
                                variant="link">
                                <text className={'hidden lg:block'}>Еще</text>
            ...
            <div className={'self-center flex gap-10 '}>
                <div
                    className={'flex h-12 w-12 self-center hover:bg-gray-300 dark:hover:bg-gray-600 rounded-full transition-all p-2'}>
                    <ThemeSwitch/>
                </div>
                {isLoggedIn && user ?
